...
def integration_step_nabla_log_p_t(y_k, nabla_log_p_t, t_k_p_1, t_k, sigma, alpha, use_exponential_integrator):
    ...
    if use_exponential_integrator and alpha.can_use_ei:
        ...
        if alpha.is_classic:
            ...
            y_k_p_1 += (ratio_pow_ap1_over_2 * factor - 1.) * torch.square(sigma) * \
                t_k * nabla_log_p_t(y_k, t_k, sigma, alpha)
            if alpha.is_case_11:
                y_k_p_1 += sigma * math.sqrt((t_k_p_1_over_T / (1. - t_k_p_1_over_T)) *
                                             (-t_k_p_1 * math.log(ratio) + alpha.T * (ratio - 1.))) * torch.randn_like(y_k_p_1)
            elif alpha.is_case_21:
                y_k_p_1 += sigma * t_k_p_1 * math.sqrt((ratio - 1.) / (1. - t_k_p_1_over_T)) * math.sqrt(
                    0.5 * ((t_k_p_1 + t_k) / (t_k * t_k_p_1)) - (1. / alpha.T)) * torch.randn_like(y_k_p_1)
            elif alpha.is_case_105:
                factor_new = (math.atanh(math.sqrt(1. - t_k_p_1_over_T)) -
                              math.atanh(math.sqrt(1. - t_k_over_T))) / math.sqrt(alpha.T * (alpha.T - t_k_p_1))
                ...
                y_k_p_1 += sigma * math.sqrt(delta * (t_k_p_1_over_T**2 + ratio) - 2. * t_k_p_1 *
                                             t_k_p_1_over_T * math.log(ratio)) * torch.randn_like(y_k_p_1) / (1. - t_k_p_1_over_T)
            else:
...
def integration_step_mc_est_grad(y_k, mc_est, t_k_p_1, t_k, sigma, alpha, use_exponential_integrator):
    ...
    if use_exponential_integrator and alpha.can_use_ei:
        ...
        if alpha.is_classic:
            ...
            y_k_p_1 = ratio * factor * y_k + (ratio * factor - 1.) * torch.square(sigma) * \
                t_k * mc_est(y_k, t_k, sigma, alpha) / alpha.alpha(t_k)
            y_k_p_1 += sigma * math.sqrt((t_k_p_1_over_T / (1. - t_k_p_1_over_T)) *
                                         (-t_k_p_1 * math.log(ratio) + alpha.T * (ratio - 1.))) * torch.randn_like(y_k_p_1)
        else:
            ratio_pow_ap1_over_2 = pow(ratio, (alpha.a + 1.) / 2.)
            y_k_p_1 = ratio_pow_ap1_over_2 * y_k
            y_k_p_1 += (ratio_pow_ap1_over_2 - 1.) * torch.square(sigma) * \
                mc_est(y_k, t_k, sigma, alpha) / pow(t_k, (alpha.a - 1.) / 2.)
            y_k_p_1 += sigma * math.sqrt(t_k_p_1 * (pow(ratio, alpha.a) - 1.0) / alpha.a) * torch.randn_like(y_k_p_1)
    else:
        log_alpha_dot_k = alpha.log_alpha_dot(t_k)
        y_k_p_1 = (1. + log_alpha_dot_k * delta) * y_k
        y_k_p_1 += delta * log_alpha_dot_k * \
            torch.square(sigma) * t_k * mc_est(y_k, t_k, sigma, alpha) / alpha.alpha(t_k)
        y_k_p_1 += sigma * math.sqrt(delta) * torch.randn_like(y_k_p_1)
    return y_k_p_1
...
def integration_step_mc_est_reparam(y_k, mc_est, t_k_p_1, t_k, sigma, alpha, use_exponential_integrator):
    ...
    if use_exponential_integrator and alpha.can_use_ei:
        ...
        if alpha.is_classic:
            ...
            t_k_p_1_over_T = t_k_p_1 / alpha.T
            factor = math.sqrt((1. - t_k_over_T) / (1. - t_k_p_1_over_T))
            y_k_p_1 = ratio * factor * y_k + (ratio * factor - 1.) * sigma * \
                math.sqrt(t_k) * mc_est(y_k, t_k, sigma, alpha)
            y_k_p_1 += sigma * math.sqrt((t_k_p_1_over_T / (1. - t_k_p_1_over_T)) *
                                         (-t_k_p_1 * math.log(ratio) + alpha.T * (ratio - 1.))) * torch.randn_like(y_k_p_1)
        else:
            ratio_pow_ap1_over_2 = pow(ratio, (alpha.a + 1.) / 2.)
