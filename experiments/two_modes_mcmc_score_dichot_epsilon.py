...
if __name__ == "__main__":
	...
	# Build the metric
	metric_ = lambda epsilon : metric(epsilon, device, args.batch_size, target, args.dim, score_est, score_type, sigma, alpha, args.K, T, epsilon_end)

	# Run the search
	epsilon = binary_search(metric_, args.epsilon_min, T - epsilon_end, 0.0, args.n_attemps)
	metrics = metric(epsilon, device, args.batch_size, target, args.dim, score_est, score_type, sigma, alpha, args.K, T, epsilon_end, return_metrics=True)

	# Dump the result
	filename = 'mcmc_score_dichot_search_alpha_{}_reparam_{}_grad_{}_{}.pkl'.format(args.alpha_type, str(bool(args.use_reparametrized)), str(bool(args.use_gradient_observable)), args.seed)
