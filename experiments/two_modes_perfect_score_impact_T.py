...
def run_experiment(device, dim, T_range, K_range, target, target_samples, sigma, nabla_log_p_t, batch_size, epsilon, alpha, em_only):
	"""Run the experiment
	...
	for T, K in tqdm(list(itertools.product(T_range, K_range))):
		...
		results.append({ 'K' : K, 'T' : T, **compute_metrics_mog(target.means, target.covs, samples, target_samples=target_samples) })
	return results
