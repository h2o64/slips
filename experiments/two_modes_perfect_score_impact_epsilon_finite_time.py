...
def run_experiment(device, dim, epsilon_range, epsilon_end_range, target, target_samples,
                   sigma, T, K, nabla_log_p_t, batch_size, alpha, em_only):
    """Run the experiment
    ...
    for epsilon, epsilon_end in tqdm(list(itertools.product(epsilon_range, epsilon_end_range))):
        ...
        results.append({'epsilon': epsilon, 'epsilon_end': epsilon_end, **
...
if __name__ == "__main__":
    ...
    if args.alpha_type == 'geometric_1_1':
        epsilon_end_range = torch.logspace(
            *tuple(map(lambda x: -int(x), args.epsilon_end_pow_range.split(','))), args.epsilon_end_num)
    else:
        alpha_geo_11 = AlphaGeometric(a=1.0, b=1.0)
        epsilon_end_11_min, epsilon_end_11_max = pow(10, -
                                                     int(args.epsilon_end_pow_range.split(',')[0])), pow(10, -
                                                                                                         int(args.epsilon_end_pow_range.split(',')[1]))
        ...
        epsilon_end_range = torch.logspace(
            math.log10(epsilon_end_min),
            math.log10(epsilon_end_max),
            args.epsilon_end_num)
